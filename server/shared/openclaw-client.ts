...
interface GatewayPendingRequest {
  method: string;
...
function getOpenClawConfig(): OpenClawConfig {
  ...
  if (!apiKey) {
    throw new OpenClawSocketError(
      "missing_api_key",
      "OPENCLAW_GATEWAY_TOKEN or OPENCLAW_API_KEY is required when IMPORT_AI_PROVIDER includes openclaw.",
    );
  }
...
function normalizeMessageData(data: unknown): Promise<string> {
...
function buildRequestFrame(id: string, method: string, params: Record<string, unknown>) {
...
function buildConnectParams(config: OpenClawConfig) {
  return {
    minProtocol: 3,
    maxProtocol: 3,
    client: {
...
function buildChatSendParams(config: OpenClawConfig, message: string, sessionKey: string, runId: string) {
  return {
    sessionKey,
    message,
    deliver: false,
...
function normalizeGatewayError(payload: JsonRecord, fallbackMethod = "unknown") {
  const error = payload.error;
  const errorRecord = error && typeof error === "object" ? (error as JsonRecord) : null;
  const details =
    errorRecord && errorRecord.details && typeof errorRecord.details === "object"
      ? (errorRecord.details as JsonRecord)
      : null;
  const message =
...
async function connectAndSend(config: OpenClawConfig, message: string, attempt: number): Promise<string> {
  ...
  return new Promise<string>((resolve, reject) => {
    ...
    const pending = new Map<string, GatewayPendingRequest>();
    ...
    const fail = (error: OpenClawSocketError) => {
      if (settled) {
        return;
      }
    ...
    const sendRequest = (method: string, params: Record<string, unknown>) => {
      ...
      const promise = new Promise<unknown>((requestResolve, requestReject) => {
        pending.set(id, {
          method,
      ...
      try {
        ...
        if (payload.type === "res" && typeof payload.id === "string") {
          ...
          if (payload.ok === true) {
            ...
            request.reject(normalizeGatewayError(payload, request.method));
          }
        ...
        if (payload.type === "event") {
          const chatEvent = parseChatEvent(payload);
...
export async function sendMessage(message: string): Promise<string> {
  ...
  let lastError: OpenClawSocketError | null = null;
